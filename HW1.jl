...
# ╔═╡ 8cbcbf1e-13f4-11eb-0635-7337837f6995
using LinearAlgebra
...
function create_bar()
	zero_vec = rand([0],100)
	mid_pos = length(zero_vec)÷2
	zero_vec[mid_pos-10:mid_pos+10] .= 1
	return zero_vec
end
...
function mean_colors(image)
	...
	return (mean(reds),mean(greens),mean(blues))
end
...
begin
	...
	function quantize(image::AbstractMatrix)
		...
		for pixel in image_copy
			push!(matrix,quantize(pixel))
		end
		return reshape(matrix,(size(image)[1],size(image)[2]))
	end
end
...
function invert(color::AbstractRGB)
	inverse = RGB(1-color.r, 1-color.g, 1-color.b)
	return inverse
end
...
# ╔═╡ d1b2d830-1273-11eb-01f6-f7d9a37437bc
function myclamp(x)
	if x > 1
		x = 1
	elseif x < 0
...
begin
	function noisify(x::Number, s)
		return myclamp(x + rand(-s:.1:s))
	end
	
	function noisify(color::AbstractRGB, s)
		noisy_color = RGB(noisify(color.r,s), noisify(color.g,s), noisify(color.b,s))
		return noisy_color
	end
...
The image is unrecognisable with intensity 1.4
"""
...
function blur_1D(v, l)
	v_copy = copy(v)
	mean_vector = []
	for i_point in 1:length(v)
		for l_point in i_point-l:i_point+l
			push!(mean_vector,extend(v,l_point))
		end
		v_copy[i_point] = mean(mean_vector)
		mean_vector = []
	end
	return v_copy
end
...
# ╔═╡ 9ff602c0-130b-11eb-1de4-b18879dc685f
@bind l Slider(0:10, show_value=true)

# ╔═╡ 60c519b2-130b-11eb-31e7-c12463d92e83
colored_line(v)
...
function convolve_vector(v, k)
	v_converted = convert(Array{Float64,1},v)
	l = (length(k) - 1) ÷ 2
	n = length(v)
	v_copy = copy(v_converted)
	for i_point in 1:n
		sum_vector = 0
		for n_point in -l:l
			sum_vector += extend(v_converted,i_point-n_point) * k[n_point+(l+1)]
		end
		v_copy[i_point] = sum_vector
	end
	return v_copy
end
...
# ╔═╡ 48ed7d92-132a-11eb-1b7b-ed25e088f53a
function gaussian(x)
	σ = 1
...
# ╔═╡ 0ec97da0-1341-11eb-17b1-8d6dff7f9557
@bind gaussian_kernel_size_1D Slider(1:100, show_value=true)
...
function extend_mat(M::AbstractMatrix, i, j)
	...
	if 1 <= i <= num_rows && 1 <= j <= num_columns
		return M[i,j]
	elseif 1 <= i <= num_rows &&  j > num_columns
		return M[i,num_columns]
	elseif 1 <= i <= num_rows && j < 1 
		return M[i,1]
	elseif 1 <= j <= num_columns && i > num_rows
		return M[num_rows,j]
	elseif 1 <= j <= num_columns &&	i < 1
		return M[1,j]
	elseif i < 1 && j < 1
		return M[1,1]
	elseif i > num_rows && j < 1
		return M[num_rows,1]
	elseif i < 1 && j > num_columns
		return M[1,num_columns]
	elseif i > num_rows && j > num_columns
...
# ╔═╡ 3673aa40-13cf-11eb-22fe-6f45b0519454
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in 1:i_max,j in 1:j_max
		sum_vector = []
		for i_test in -l_i:l_i,j_test in -l_j:l_j
			push!(sum_vector,
				extend_mat(M,i+i_test,j+j_test) * K[i_test+(num_rows-1),j_test+(num_columns-1)])
		end
		M_copy[i,j] = sum(sum_vector)
	end
	return M_copy
end
...
Gray.(convolve_image(test_image_with_border, K_test))
...
# ╔═╡ a6019620-13f1-11eb-3961-abda8c55f17d
function gaussian_2D(x,y)
	σ = 1
	return (1 / √(2*π*σ^2)) * (exp(-(x^2+y^2)/2*σ^2))
end

# ╔═╡ cb56f860-13f2-11eb-1b2d-19a62a064d2a
function gaussian_kernel_2D(n)
	kernel = zeros(n,n)
	l_i = (size(kernel)[1] - 1) ÷ 2
	l_j = (size(kernel)[2] - 1) ÷ 2
	for i in -l_i:l_i, j in -l_j:l_j
		kernel[i+(l_i+1),j+(l_j+1)] = gaussian_2D(i,j)
	end
	return normalize(kernel,1)
end
...
# ╔═╡ fcb3b38e-13f6-11eb-2cec-c78aa3cf4caa
[test_image_with_border with_gaussian_blur(test_image_with_border)]

# ╔═╡ b290d1d0-13f6-11eb-1968-31d5de544fc6
...
# ╔═╡ dfbc8d80-13fa-11eb-2d17-a91db2f9ce51
begin
	matrix_x = [1 0 -1 ; 2 0 -2 ; 1 0 -1]
	matrix_y = [1  2  1 ; 0  0  0 ; -1 -2 -1]	

	[3 * Gray.(convolve_image(abs.(philip),normalize(matrix_x,1))) 3 * Gray.(convolve_image(abs.(philip),normalize(matrix_y,1)))]
end


# ╔═╡ 9eeb876c-ee15-11ea-1794-d3ea79f47b75
function with_sobel_edge_detect(image)
	Gx = convolve_image(abs.(image),normalize(matrix_x,1))
	Gy = convolve_image(abs.(image),normalize(matrix_y,1))
	G_total = sqrt.((Gx.^2+Gy.^2))
	return Gray.(G_total)
end

# ╔═╡ 61bb86c0-13ff-11eb-0352-0df641258d67
