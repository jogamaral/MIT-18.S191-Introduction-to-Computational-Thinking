...
student = (name = "João Gabriel", kerberos_id = "jazz")
...
ex_1_1 = md"""
Because if our guess `a` is greater than $\sqrt{x}$ then `x/a` will be smaller or equal $\sqrt{x}$

And  if our guess `a` is smaller than $\sqrt{x}$ then `x/a` will be greater or equal $\sqrt{x}$
""" 

# you might need to wait until all other cells in this notebook have completed running. 
# scroll down the page to see what's up

# ╔═╡ 688cb900-1090-11eb-2f17-75e998f0bdfe
...
# ╔═╡ 83866f20-1091-11eb-3c14-b929e70e3e00
begin
	x = 2
	a = 1
	abs(x/a - a) > 0.01
